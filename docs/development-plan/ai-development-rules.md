...
- 仓库级资源类文件必须统一放在 `resources/` 下的分类子目录中，禁止直接放在 `resources/` 根目录。数据库默认放入 `resources/database/`，全局日志默认放入 `resources/logs/`，全局配置默认放入 `resources/global/`；新增资源类型必须先建对应分类目录并同步文档。
