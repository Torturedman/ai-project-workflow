...
**Tech Stack:** Node.js 24 LTS、TypeScript 5.9 稳定线、npm、Commander、Zod、Execa、Pino、YAML、Vitest、Playwright、SQLite。MVP 首先实现 `node-next` Profile，随后补齐 `python-fastapi-react` 与 `java-spring-vue`。精确依赖版本以 `docs/development-plan/dependency-versions.md` 为准，版本选择遵循稳定兼容基线，不盲目追最新主版本。
...
- [ ] 按 `docs/development-plan/dependency-versions.md` 锁定主控依赖版本。
...
- [ ] 实现三个内置 Profile。
- [ ] 按 `docs/development-plan/dependency-versions.md` 为每个 Profile 声明运行时和依赖版本。
...
- [ ] 生成 Next.js 项目骨架。
- [ ] 按 `docs/development-plan/dependency-versions.md` 锁定 Next.js、React、Prisma、Playwright 等版本。
...
- [ ] 生成目录骨架和配置文件。
- [ ] 按 `docs/development-plan/dependency-versions.md` 锁定 Python 和前端依赖版本。
