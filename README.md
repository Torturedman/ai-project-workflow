...
仓库级资源统一放在 `resources/` 的分类子目录中：SQLite 索引库默认路径为 `resources/database/ai-factory.db`，全局日志默认路径为 `resources/logs/ai-factory-global.jsonl`，全局配置默认路径为 `resources/global/config.yaml`。不要直接把文件放到 `resources/` 根目录。
